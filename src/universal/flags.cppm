...
class Flags final {
 public:
  ...
  /*implicit*/ constexpr Flags(Enum value) noexcept
      : value_(static_cast<ValueType>(value)) {}

  constexpr Flags(std::initializer_list<Enum> values) noexcept : Flags() {
  ...
  constexpr Flags& Clear(Flags flags) {
    value_ &= ~flags.value_;
    return *this;
  }

  constexpr Flags<Enum> operator|(Flags rhs) const {
  ...
  constexpr bool operator==(Flags rhs) const { return value_ == rhs.value_; }

  constexpr bool operator!=(Flags rhs) const { return !(*this == rhs); }

  constexpr ValueType GetValue() const { return this->value_; }

  constexpr void SetValue(ValueType value) { this->value_ = value; }
...
//   auto expected_int = expected.GetValue();
//   const bool result =
//       value_.compare_exchange_strong(expected_int, desired.GetValue(),
//       order);
//   expected = Enum{expected_int};
//   return result;
// }
