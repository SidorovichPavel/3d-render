...
namespace engine {
    ...
    public:
        ...
        mdspan(T* ptr, Args&&... args) noexcept : ptr_(ptr) {
            static_assert((std::is_same_v<std::remove_cvref_t<Args>, std::size_t> && ...), "fail");
        ...
        mdspan<T, (Dims - 1)> operator[](std::size_t offset) noexcept {
            ...
            assert(ptr_);
            std::size_t block_size = std::accumulate(std::next(dims_size.begin()), dims_size.end(), 1u, std::multiplies<std::size_t>());
            auto ptr_with_offset = ptr_ + offset * block_size;
            return mdspan<T, (Dims - 1)>(ptr_with_offset, std::ranges::subrange(std::next(dims_size.begin()), dims_size.end()));
        }
